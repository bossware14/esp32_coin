...
class WifiManager:
    ...
    def __init__(self, ssid = 'ESP32', password = '123456789', reboot = True, debug = False):
    ...
    def handle_root(self):
        ...
        self.client.sendall("""
                        ...
                        <br>
                        <p><label for="gpio">Gpio:&nbsp;</label><input type="text" id="gpio" name="gpio" value="14"></p>
                        <br>
                        <p><label for="device">ชื่อ:&nbsp;</label><input type="text" id="device" name="device" value=""></p>
    ...
    def handle_configure(self):
        match = re.search('ssid=([^&]*)&password=(.*)&gpio=(.*)&device=(.*)', self.url_decode(self.request))
        if match:
            ssid = match.group(1).decode('utf-8')
            password = match.group(2).decode('utf-8')
            gpio = match.group(3).decode('utf-8')
            device = match.group(4).decode('utf-8')
            if len(ssid) == 0:
                self.send_response("""
                ...
                data = {"version":5,"gpio":gpio,"autoload":2,"sendcoin":0,"url":"https://esp32-th-default-rtdb.asia-southeast1.firebasedatabase.app/esp32/"}
